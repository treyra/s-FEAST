...
def predictAndUpdateAll(nominalAction,observation,previousFilters,possibleFailures,systemF,systemParametersTuple,physicalStateJacobianF,
    ...
    batchPredictFilter = jax.vmap(predictNonLinear,in_axes=[0, None, 0, None, None,None])
...
def makeGeneralFaultExtendedKalmanFilter():
    """
...
def predictNonLinear(previousFilter,nominalAction,failureState,systemF,systemParametersTuple,physicalStateJacobianF):
    """
    ...
    jacobian = physicalStateJacobianF(physicalState=previousMean,failureState=failureState,nominalAction=nominalAction,systemParametersTuple=systemParametersTuple)
